...
export class Checkout {
  ...
  scan(productSKU: string): void {
    const productPrice = this.productPriceRules.find((x) => x.isApplicable(productSKU))
  ...
  recordNewScan(productPrice: ProductPriceStrategy, existingScan: ScanRecord | undefined): ScanRecord {
    if (!existingScan) return { quantity: 1, price: productPrice.calculatePrice(1) }
  ...
  private calculateTotalPrice(scans: Record<string, ScanRecord>): number {
    return Object.values(scans).reduce((accumulator, scan) => accumulator + scan.price, 0)
  }
}
